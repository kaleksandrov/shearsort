...
public class MergeSort {
	...
	private static Boolean mergeSort(int[] src, int[] dest, int low, int high,
			int off) {
		int length = high - low;
		...
		wasArraySorted = mergeSort(dest, src, mid, high, -off)
				&& wasArraySorted;
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private static Boolean mergeSort(int[] src, int[] dest, int low, int high,
			int off, Comparator comparator) {
		int length = high - low;
		Boolean wasArraySorted = true;

		// Insertion sort on smallest arrays
		if (length < INSERTIONSORT_THRESHOLD) {
			wasArraySorted = InsertionSort.sort(dest, comparator)
					&& wasArraySorted;
			return wasArraySorted;
		}
		...
		high += off;
		int mid = (low + high) >>> 1;
		wasArraySorted = mergeSort(dest, src, low, mid, -off, comparator)
				&& wasArraySorted;
		wasArraySorted = mergeSort(dest, src, mid, high, -off, comparator)
				&& wasArraySorted;
